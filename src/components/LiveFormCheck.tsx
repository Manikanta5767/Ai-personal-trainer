  ...
  useEffect(() => {
    ...
    const initMediaPipe = async () => {
      ...
      const onResults = (results: any) => {
        ...
        // ── Posture angle examples ────────────────────────────────────────
        // LEFT side — hip angle (shoulder → hip → knee)
        ...
        // LEFT side — knee angle (hip → knee → ankle)
        ...
        console.log(
          `[LiveFormCheck] Left hip angle: ${leftHipAngle}° | Left knee angle: ${leftKneeAngle}°`
        );

        // ── Annotate angles on canvas ─────────────────────────────────────
        ...
        drawAngleLabel(lm[POSE_LANDMARKS.LEFT_HIP], leftHipAngle, "Hip");
        drawAngleLabel(lm[POSE_LANDMARKS.LEFT_KNEE], leftKneeAngle, "Knee");

        // Custom posture thresholds
        if (leftHipAngle < 80) {
          setPostureStatus({ label: "⚠️ Back rounding detected", color: "text-destructive" });
        } else if (leftKneeAngle < 90) {
          setPostureStatus({ label: "⚠️ Knee over-extension", color: "text-yellow-400" });
        } else {
          setPostureStatus({ label: "✅ Good form", color: "text-primary" });
        }
      };
